...
#
# Stamps two per-window options consumed by the window-status formats in
# ~/.tmux.conf: @agent_style (normal windows, dark background) and
# @agent_style_cur (current window, light background).
#
...
# Per-agent-pane records: "window_id busy|idle". Busy = pane title starts with
...
records=$(tmux list-panes -a -F '#{window_id}|#{pane_pid}|#{pane_title}' |
  while IFS='|' read -r win pid title; do
